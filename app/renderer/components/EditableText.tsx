    ...
    if (props.keywords) {
      ...
      props.keywords.map((keyword) => {
        words.map((word, index) => {
          ...
          if (valid) {
            words[index] = word.replace(keyword.matcher, '<span class="valid">$&</span>')
          } else {
            words[index] = word.replace(keyword.matcher, '<span class="invalid">$&</span>')
          }
        })
      })
    ...
    <ThemeProvider theme={theme}>
      ...
      >
        ...
        {input.length == 0 &&
          ...
          !editable && <Placeholder onClick={handleClick}>{props.placeholder}</Placeholder>}
      </div>
    </ThemeProvider>
  )
}
