  ...
  (state = initialState, action: ui.UIActions): UIType => {
    switch (action.type) {
      ...
      case ui.TOGGLE_SUBTASKS:
        ...
        if (itemVisiblity != undefined) {
          ...
          if (componentVisibility != undefined) {
            state.subtasksVisible[action.id][action.componentId] = !state.subtasksVisible[
              action.id
            ][action.componentId]
          } else {
            state.subtasksVisible[action.id][action.componentId] = false
      ...
      case ui.SET_FILTEREDITEMLIST_NAME:
        if ((state.components.components[action.componentId].component.name = 'FilteredItemList')) {
          state.components.components[action.componentId].component.props.listName = action.name
        }
        break

      case ui.SET_FILTEREDITEMLIST_FILTER:
        if ((state.components.components[action.componentId].component.name = 'FilteredItemList')) {
          state.components.components[action.componentId].component.props.filter = action.filter
        }
        break
