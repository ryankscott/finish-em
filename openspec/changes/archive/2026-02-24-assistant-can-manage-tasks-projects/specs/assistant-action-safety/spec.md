...
The system SHALL produce normalized per-action outcomes that include action type, target reference (if available), status, and human-readable message suitable for assistant UI feedback.
...
#### Scenario: Successful single action confirmation
- **WHEN** the assistant executes one valid mutation action successfully
- **THEN** the system returns a success outcome with a concise confirmation message that can be shown directly in assistant UI
