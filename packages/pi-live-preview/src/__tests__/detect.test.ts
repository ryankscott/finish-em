...
describe("parsePortFromOutput", () => {
  test("finds localhost:PORT", () => {
    expect(parsePortFromOutput("Server running at http://localhost:5173/")).toBe(5173);
  });
  ...
  test("finds python http.server output", () => {
    expect(parsePortFromOutput("Serving HTTP on 0.0.0.0 port 8000 (http://0.0.0.0:8000/) ...")).toBe(8000);
  });
...
describe("detectUrl", () => {
  ...
  test("tier 3: output parsing", () => {
    const output = "ready started server on 0.0.0.0:9876, url: http://localhost:9876";
    expect(detectUrl("some-unknown-tool run", output)).toBe("http://localhost:9876");
  });
