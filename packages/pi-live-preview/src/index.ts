...
import { COMMON_PORTS, detectUrl, isDevServerCommand } from "./detect";
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description:
			...
			"If no URL is provided, automatically detects common dev server ports.",
		promptSnippet: "Open a live browser preview of a running web app",
		promptGuidelines: [
		...
		renderResult(result, { expanded, isPartial }, theme, context) {
			...
			if (details?.url) {
				let txt = theme.fg(
					"success",
					details.reused ? "Preview updated" : "Preview opened",
				);
				if (expanded) {
					txt += `\n  ${theme.fg("dim", `URL: ${details.url}`)}`;
				}
	...
	pi.registerTool({
		...
		async execute() {
			...
			return {
				content: [
					{ type: "text", text: `Preview closed: ${parts.join(", ")}.` },
				],
	...
	pi.on("tool_result", async (event) => {
		...
		const output =
			event.content?.[0] && "text" in event.content[0]
