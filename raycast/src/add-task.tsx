...
export default function AddTask() {
  const { data: projects, isLoading } = useCachedPromise(() => runCli<Project[]>("project list --json"));

  async function handleSubmit(values: { title: string; projectId?: string; priority?: string }) {
    const flags: string[] = ["--json"];
    const selectedProjectId = values.projectId?.trim();
    const fallbackProjectId = projects?.find((project) => project.isInbox)?.id ?? projects?.[0]?.id;
    const projectIdToUse = selectedProjectId || (fallbackProjectId ? String(fallbackProjectId) : "");
    ...
    try {
      const task = await runCli<Task>(`task add "$TASK_TITLE" ${flags.join(" ")}`, {
        TASK_TITLE: values.title,
      });
      await showToast({ style: Toast.Style.Success, title: `Added: ${task.title}` });
      await popToRoot();
    } catch (err) {
      await showToast({
  ...
  return (
    ...
    >
      ...
      <Form.Dropdown id="projectId" title="Project" defaultValue="">
        ...
        {(projects ?? []).map((p) => (
          <Form.Dropdown.Item key={p.id} value={String(p.id)} title={p.emoji ? `${p.emoji} ${p.name}` : p.name} />
        ))}
      </Form.Dropdown>
