...
export function parseTaskEditInput(
	...
	if (recursMatch) {
		...
		if (
			...
			warnings.push(
				`Unknown recurrence preset "${rawPreset}". Use: daily, weekly, monthly, yearly, every_weekday, none`,
			);
		}
		working = working
	...
	if (dueMatch && dueMatch.index !== undefined) {
		...
		const [phrase, end] = extractTokenValue(working, valueStart, {
			...
			extraStopPattern: /\s+p[1-4](?:\s|$)/,
		});
		const parsed = parseDatePhrase(phrase);
		if (parsed === undefined) {
			warnings.push(
				`Unrecognised due date "${phrase}". Use: today, tomorrow, next week, monday–sunday, DD/MM/YY, YYYY-MM-DD, none`,
			);
		} else {
			patch.dueAt = parsed;
			if (parsed !== null) {
	...
	if (scheduledMatch && scheduledMatch.index !== undefined) {
		...
		if (parsed === undefined) {
			warnings.push(
				`Unrecognised scheduled date "${phrase}". Use: today, tomorrow, next week, monday–sunday, DD/MM/YY, YYYY-MM-DD, none`,
			);
		} else {
	...
	if (projectMatch && projectMatch.index !== undefined) {
		...
		working = (working.slice(0, projectMatch.index) + working.slice(valueEnd))
			.replace(/\s{2,}/g, " ")
			.trim();
	}
	...
	if (parentMatch && parentMatch.index !== undefined) {
		...
		if (["none", "clear", "null"].includes(normalized)) {
			...
			if (Number.isInteger(asNumber) && asNumber > 0) {
				...
				warnings.push(
					`Invalid parent value "${parentValue}". Use: parent:<taskId> or parent:none`,
				);
			}
		}
