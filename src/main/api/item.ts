...
export const getFilteredItems = async (
  ...
  if ((await results.length) >= 0) {
    ...
    const orders = await getItemOrdersByComponent(
    ...
    await deleteItemOrders(
      { itemKeys: inOrderButNotResult, componentKey: input.componentKey },
      ctx
    );
...
export const createMigrateItemQuery = (input: {
  ...
  const lastUpdatedText = input.lastUpdatedAt
    ...
    : null;
  const completedText = input.completedAt
    ? `'${input.completedAt.toISOString()}'`
    : null;
  const deletedText = input.deletedAt
    ? `'${input.deletedAt.toISOString()}'`
    : null;
  const createdText = input.createdAt
...
export const setRepeatOfItem = (
  input: { key: string; repeat: string },
  ctx
) => {
  return ctx.db
    .run(
...
export const setProjectOfItem = async (
  input: { key: string; projectKey: string },
  ctx
) => {
  return ctx.db
    .run(
...
export const setScheduledAtOfItem = (
  input: { key: string; scheduledAt: Date },
  ctx
) => {
  return ctx.db
    .run(
...
export const setParentOfItem = async (
  input: { key: string; parentKey: string },
  ctx
) => {
  ...
  if (children.length > 0) {
    log.error(
      `Unable to set parent of item as it's already a parent, key - ${input.key}`
    );
...
export const permanentDeleteItem = (input: { key: string }, ctx) => {
  return ctx.db
    .run(SQL`DELETE from item WHERE key = ${input.key}`)
    .then((result) => {
      if (result.changes) {
        return input.key;
      }
...
export const setLabelOfItem = (
  input: { key: string; labelKey: string },
  ctx
) => {
  return ctx.db
    .run(
