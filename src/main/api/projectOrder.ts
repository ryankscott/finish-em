...
export const getProjectOrders = (obj, ctx) => {
  return ctx.db
    ...
    .then((result) =>
      result.map((r) => new ProjectOrder(r.projectKey, r.sortOrder))
    );
};
...
export const getProjectOrder = (input: { projectKey: string }, ctx) => {
  return ctx.db
    .get(
      `SELECT projectKey, sortOrder FROM projectOrder WHERE projectKey = '${input.projectKey}'`
    )
    .then((result) => new ProjectOrder(result.projectKey, result.sortOrder));
};

export const setProjectOrder = async (
  input: { projectKey: string; sortOrder: number },
  ctx
) => {
  try {
    const currentProjectOrder = await getProjectOrder(
...
export const createCreateProjectOrderQuery = (input: {
  projectKey: string;
}) => {
...
export const createMigrateProjectOrder = (input: {
  projectKey: string;
  sortOrder: number;
}) => {
