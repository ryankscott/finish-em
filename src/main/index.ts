...
const determineDatabasePath = (): string => {
  ...
  return isDev ? './finish_em.db' : path.join(app.getPath('userData'), './finish_em.db')
}
...
const startApolloServer = async () => {
  const schemasPath = isDev
    ? path.join(__dirname, '../../resources/schemas/')
        ...
        const sortedReleases = resp
          ...
          .sort((a, b) => {
            if (a.published_at && b.published_at) {
              return isAfter(parseISO(a.published_at), parseISO(b.published_at)) ? -1 : 1
            }
            return 0
          })
...
const startApp = async () => {
  ...
  if (!enabled) {
    ...
    const calendarIntegration = features?.find((f) => f.name === 'calendarIntegration')
