...
export default function CalendarAgenda({ selectedDate }: CalendarAgendaProps) {
  ...
  const sortedEventsForToday: Event[] = getSortedEventsForToday(events, selectedDate)
  ...
  return (
    <>
        ...
        return (
          ...
          >
            ...
            >
              ...
              >
                {format(sub(parseISO(e.startAt ?? ''), { minutes: offset }), 'h:mm aa')}
              </Text>
      ...
      <EventModal isOpen={showModal} onClose={() => setShowModal(false)} event={activeEvent} />
    </>
  )
}
