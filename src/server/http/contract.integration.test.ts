...
for (const [name, makeClient] of clients) {
  describe(`api contract (${name})`, () => {
    it('reads and updates settings', async () => {
      ...
      const updated = await api.updateSettings({ timezone: 'America/New_York' })
    ...
    it('supports the task lifecycle: create, update, complete, delete, undelete', async () => {
      ...
      const updated = await api.updateTask(task.id, { title: 'Ship it now', priority: 2 })
    ...
    it('filters tasks by query params', async () => {
      ...
      const parent = await api.createTask({ projectId: project.id, title: 'Parent', priority: 3 })
      ...
      const roots = await api.listTasks({ projectId: project.id, rootsOnly: true })
    ...
    it('supports goals', async () => {
      ...
      const goals = await api.listGoals({ periodType: 'weekly', periodStart: '2026-06-08' })
    ...
    it('supports reminders with task titles', async () => {
      ...
      const task = await api.createTask({ projectId: project.id, title: 'Call vet' })
