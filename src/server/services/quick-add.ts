...
function deterministicParse(rawInput: string): QuickAddParseResult {
  ...
  const priority = priorityMatch ? (Number(priorityMatch[1]) as Priority) : null
  if (priorityMatch) {
  ...
  const scheduleMatch = working.match(/\b(start|scheduled)\s+(today|tomorrow|next week|\d{4}-\d{2}-\d{2}(?:\s+at\s+.+)?)/i)
  if (scheduleMatch) {
  ...
  const dueMatch = working.match(/\b(due\s+)?(today|tomorrow|next week|\d{4}-\d{2}-\d{2}(?:\s+at\s+.+)?)/i)
  if (dueMatch) {
  ...
  if (!dueAt && !scheduledAt && !recurrencePreset && !recurrenceRRule && !projectName && !priority) {
    confidence -= 0.25
  }
