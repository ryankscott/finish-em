...
export function createHttpApi(
  ...
  return {
    ...
    updateTask: (taskId, input) => request('PATCH', `/api/tasks/${taskId}`, input),
    ...
    uncompleteTask: (taskId) => request('POST', `/api/tasks/${taskId}/uncomplete`),

    listGoals: (query = {}) => request('GET', `/api/goals${queryString(query)}`),
    createGoal: (input) => request('POST', '/api/goals', input),
    updateGoal: (goalId, input) => request('PATCH', `/api/goals/${goalId}`, input),
    deleteGoal: async (goalId) => {
      await request('DELETE', `/api/goals/${goalId}`)
    },

    listTaskReminders: (taskId) => request('GET', `/api/tasks/${taskId}/reminders`),
