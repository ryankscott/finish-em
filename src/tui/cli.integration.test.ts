...
describe('runCli integration', () => {
  ...
  it('preserves project deletion semantics through CLI', async () => {
    ...
    const createdProject = await api.createProject({ name: 'CLI Delete', color: '#ef4444' })
    await api.createTask({
      title: 'Task in deleted project',
      projectId: createdProject.id,
    })

    const deleted = await runWithCapture(['project', 'delete', String(createdProject.id), '--json'])
    ...
    expect(inboxTasks.some((task) => task.title === 'Task in deleted project')).toBe(true)

    const failInbox = await runWithCapture(['project', 'delete', String(inbox!.id)])
  ...
  it('supports blocking and unblocking tasks through CLI', async () => {
    ...
    const blockedList = await runWithCapture(['task', 'list', '--blocked', 'true'])
    ...
    const blockedDone = await runWithCapture(['task', 'done', String(task!.id)])
    expect(blockedDone.result.exitCode).toBe(1)
    expect(blockedDone.stderr).toContain('Blocked tasks cannot be completed until unblocked')
