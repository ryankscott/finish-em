...
const formatProjectList = (
	projects: Array<{ id: number; name: string; isInbox: boolean }>,
) => {
	if (projects.length === 0) return "No projects found.";
	return projects
		.map(
			(project) =>
				`[${project.id}] ${project.name}${project.isInbox ? " (inbox)" : ""}`,
		)
		.join("\n");
};

const formatGoalList = (
	goals: Array<{
		id: number;
		done: boolean;
		title: string;
	...
	return goals
		.map(
			(goal) =>
				`[${goal.id}] ${goal.done ? "done" : "open"} ${goal.periodType} ${goal.periodStart} ${goal.title}`,
		)
		.join("\n");
};
...
const formatReminderList = (
	...
	return reminders
		.map(
			(reminder) =>
				`[${reminder.id}] task=${reminder.taskId} ${reminder.status} remindAt=${reminder.remindAt}`,
		)
		.join("\n");
};
...
async function runProjectCommand(args: string[], api: ApiClient, io: CliIo) {
	...
	if (subcommand === "list") {
		...
		printResult(io, projects, {
			json: outputJson,
			human: formatProjectList(projects),
		});
		return;
	}
	...
	if (subcommand === "update") {
		const projectId = Number.parseInt(
			requirePositional(positionals, 1, "project-id"),
			10,
		);
...
async function runGoalCommand(args: string[], api: ApiClient, io: CliIo) {
	...
	if (subcommand === "list") {
		const goals = await api.listGoals({
			periodType: asString(flags, "period-type") as
				| "daily"
				| "weekly"
				| undefined,
	...
	if (subcommand === "add") {
		const title = requirePositional(positionals, 1, "title");
		const periodType = asString(flags, "period-type") as
			| "daily"
			| "weekly"
			| undefined;
		...
		if (!periodType || !periodStart) {
			throw new Error(
				"Missing required flags: --period-type and --period-start",
			);
		}
		...
		printResult(io, goal, {
			json: outputJson,
			human: `Created goal [${goal.id}] ${goal.title}`,
		});
		return;
	}

	if (subcommand === "update") {
		const goalId = Number.parseInt(
			requirePositional(positionals, 1, "goal-id"),
			10,
		);
		...
		printResult(io, goal, {
			json: outputJson,
			human: `Updated goal [${goal.id}] ${goal.title}`,
		});
		return;
	}
	...
	if (subcommand === "delete") {
		...
		printResult(
			io,
			{ ok: true, goalId },
			{ json: outputJson, human: `Deleted goal ${goalId}` },
		);
		return;
	}
...
async function runReminderCommand(args: string[], api: ApiClient, io: CliIo) {
	...
	if (subcommand === "list") {
		...
		printResult(io, reminders, {
			json: outputJson,
			human: formatReminderList(reminders),
		});
		return;
	}
	...
	if (subcommand === "delete") {
		const reminderId = Number.parseInt(
			requirePositional(positionals, 1, "reminder-id"),
			10,
		);
