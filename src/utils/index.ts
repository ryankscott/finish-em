...
export const createShortProjectName = (input: string): string => {
    ...
    if (words.length == 1) {
        const word = words[0]
        // If the first word has an emoji, return the emoji
        if (hasEmoji([...word][0])) {
            return [...word][0]
        }
    ...
    // If there are emojis in any word just return the emoji
    const containsEmoji = words.filter((w) => hasEmoji(w))
    if (containsEmoji.length > 0) {
        return getEmoji(containsEmoji[0])
    }
