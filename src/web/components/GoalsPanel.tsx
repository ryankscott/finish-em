...
function GoalRow({ goal }: { goal: Goal }) {
  ...
  const toggle = () =>
    updateGoal.mutate(
      { goalId: goal.id, input: { done: !goal.done } },
  ...
  return (
    <div className="group flex items-center gap-2 rounded-md px-2 py-1 text-sm hover:bg-surface">
      <button type="button" onClick={toggle} aria-label="Toggle goal" className="shrink-0">
        {goal.done ? (
          <CheckCircle2 className="h-4 w-4 text-p3" />
        ) : (
      ...
      {editing ? (
        ...
        <span className={cn('min-w-0 flex-1 truncate', goal.done && 'text-muted line-through')}>
          {goal.title}
        </span>
      )}
      <button
      ...
      <button
        ...
        onClick={() =>
          deleteGoal.mutate(goal.id, { onError: (err) => toast.error(err.message) })
        }
...
export function GoalsPanel({
  ...
  return (
    <div className="w-64 shrink-0 border-l border-border p-3">
