...
function ProjectNavLink({
  ...
  const onDelete = () => {
    if (!window.confirm(`Delete project "${project.name}"? Its tasks are removed too.`)) {
      return
    }
  ...
  return (
    ...
    >
      <Link to={`/projects/${project.id}`} tabIndex={-1} className="flex min-w-0 flex-1 items-center gap-2">
...
export function Sidebar() {
  ...
  const { data: blockedTasks = [] } = useTasks({ status: 'open', blocked: true })
  ...
  return (
    <aside className="flex w-60 shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-border bg-surface/50 p-2">
      ...
      />
      <div className="mt-3 mb-1 flex items-center px-3 text-[11px] font-semibold tracking-wide text-muted uppercase">
        <span>Projects</span>
        <button
          type="button"
